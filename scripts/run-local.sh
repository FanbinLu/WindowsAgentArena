...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help)
            ...
            echo "  --mode <dev/eval/azure> : Mode (default: azure)"
            exit 0
            ;;
        *)
...
bash ./run.sh --mode $mode --prepare-image $prepare_image --container-name $container_name --skip-build $skip_build --interactive $interactive --connect $connect --use-kvm $use_kvm --ram-size $ram_size --cpu-cores $cpu_cores --mount-vm-storage $mount_vm_storage --mount-client $mount_client --mount-server $mount_server --browser-port $browser_port --rdp-port $rdp_port --start-client $start_client --agent $agent --model $model --som-origin $som_origin --a11y-backend $a11y_backend --gpu-enabled $gpu_enabled --openai-api-key $OPENAI_API_KEY --openai-base-url $OPENAI_BASE_URL --azure-api-key $AZURE_API_KEY --azure-endpoint $AZURE_ENDPOINT